...
0.13.2 图片适配验收还须覆盖：豆包 Normal 状态保留 blob 预览但有匹配远端 key；DeepSeek 独立图片卡、异步 WebP 转换的原始文件到 localId 绑定，以及原始状态未被观察、身份变化、上传中或审核失败时不发送。官网账号实传验收当前待登录，必须单独记录，不能用这些回归测试代替。
