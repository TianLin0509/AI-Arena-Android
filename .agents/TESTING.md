...
修改网页发送、消息定位或回答提取时，合成 WebView 测试只验证已知网页结构，不能代替官网回归。冻结候选后，在明确指定、保留登录的隔离账号设备上完成连续使用测试：

- DeepSeek、豆包、Kimi 至少 30 轮 / 90 个回答核对；包括新对话、连续追问、相同正文重复提问、中文多段、长回答、后台返回，以及窄屏下打开原网页再返回与冷启动恢复。
- 每轮通过原生 App 输入并发送，发送前核对输入正文。只读网页观察可以用于取证，不用 CDP 或网页手工发送替代产品路径。
- 每轮核对选定成员均参与、本轮用户消息身份、App 保存与显示的答案、网页完整答案和结束标记；区分网站失败占位行与已接受的问题，检查重复提交。不能只看发送点击、输入框清空或 COMPLETE 状态。
- 失败立即留存，不自动补发后计为通过。记录 App 缺陷、网页拒绝/限流、模拟器故障、测试驱动故障各自的证据；修复改变产品代码后重新冻结并执行受影响矩阵。
- 证据绑定 APK SHA-256、源码 SHA、设备/AVD、网页测试日期、逐轮输入与核对结果；测试前后的账号数据不清除。不可将旧版测试数字或 debug 结果充当正式签名 R8 包验收。
- 本轮定位信息丢失时必须明确报错且不重复发送；页面残留旧答案不能作为本轮成功。已完成历史答案保留，恢复旧网页缺少定位信息时引导用户在原网页核对。

这些检查降低已知故障复发概率，不证明所有账号、所有网站版本或所有网络条件下绝无故障。
...
- 修改可见性、就绪判定或豆包发送时，除合成用例外，须在保留登录的隔离账号设备上跑“新对话 + 发送后不同时机切后台”的连续轮次，并用 CDP 只读记录豆包 `sendMessageStatusMap`、停止按钮与队列。

## 发布

当前产品版本 `0.15.3`、versionCode `27`，采用简洁顶部 Tab 回答界面。发布任务才更新版本；不采用每次合并自动 bump，以免普通流程变更触发用户升级。
