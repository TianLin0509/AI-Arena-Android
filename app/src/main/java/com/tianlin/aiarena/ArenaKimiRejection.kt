...
internal object ArenaKimiRejection {
    ...
    const val quotaDetail = "Kimi 网页提示账号额度已用完，问题没有发出；请打开原网页查看额度或稍后重试"
    ...
    val expression = """
        (function() {
          ...
          for (const dialog of dialogs) {
            ...
            // 2026-09-25 free account dialog: "Tips / Credits used up. / Got it / Upgrade".
            if (/Credits used up|out of credits|额度已?用完|次数已?用完|用量已达上限/i.test(words)) return 'quota';
    ...
    fun detail(rejection: String): String? = when (rejection) {
        "busy" -> busyDetail
        "quota" -> quotaDetail
