...
- 通过隔离模拟器的会话与网页夹具验证；真实厂商账号的同时上传和发送仍需专用账号验证，网页版本和风控可能影响结果。
...
- 当前圆桌附件适配 DeepSeek、豆包、Kimi；选择其他成员时明确提示尚不支持附件。已完成隔离 WebView 自动测试，真实官网登录上传尚需专用账号验证。
- 0.13.2 适配豆包当前上传菜单及附件区域，识别 DeepSeek 图片卡和手机 WebP 转换，并支持 Kimi 当前发送控件。转换后的图片必须与本次选图唯一对应且身份稳定；歧义、额外附件或身份变化会明确停止。官网账号自动上传与内容识别单独验收，不用合成网页结果代替。
